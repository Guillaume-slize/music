...
#
# ## Links from discogs to overviews of 500 most wanted disc of 2016  
base.link = "https://www.discogs.com"
all.disclinks <- paste ("https://www.discogs.com/search/?sort=want%2Cdesc&page=",seq(1,10),"&decade=2010&year=2016", sep="")


all.stylelinks<- paste ("https://reference.discogslabs.com/browse/style?page",seq(1,11), sep="")
...
css.selector.styles = ".ng-binding"

# 
# ##================= 1.4 Bulding functions (link collection and scraping) =====================
# 
## Function 1: creating collector-function that finds all the links to most wanted discs
...
##function 2: collector for style:
# style.collector = function(vector){
#   out = vector %>%
#     read_html(encoding = "UTF-8") %>% #inddryearr special danish character
#     html_nodes(css =css.selector.styles ) %>%
#     html_attr(name = 'href') #tyearr den attribut med navnet hret
#   Sys.sleep(sample(seq(1, 2, by=0.001), 1))
#   return (out)
# }
## Function 3: Creating scraper-function to scrape all  stats from detailed discs pyear
scrape_discstats = function(link){
  ...
  info2 = my.link %>%
    html_nodes(".content:nth-child(7) a") %>%
    html_text()
  info2 = info2[1]
  Sys.sleep(sample(seq(1, 2, by=0.001), 1))
  
  info4 = my.link %>%
    html_nodes(".content:nth-child(9) a") %>%
    html_text()
  info4 = info4[1]
  Sys.sleep(sample(seq(1, 2, by=0.001), 1))
  
  info5 = my.link %>%
    html_nodes(".content:nth-child(11) a") %>%
    html_text()
  info5 = info5[1]
  Sys.sleep(sample(seq(1, 2, by=0.001), 1))
  
  info6 = my.link %>%
    html_nodes(".content:nth-child(13) a") %>%
    html_text()
  info6 = info6[1]
  Sys.sleep(sample(seq(1, 2, by=0.001), 1))
...
#   Sys.sleep(sample(seq(1, 2, by=0.001), 1))
#   format = my.link %>%
#     html_nodes(".doclist-item-list:nth-child(1) .ng-binding") %>%
#     html_text()
#   format = format[1]
#   Sys.sleep(sample(seq(1, 2, by=0.001), 1))
#   return(data_frame(styler = styler,
#                     format= format))
#   Sys.sleep(sample(seq(3, 7, by=0.001), 1))
# }
...
all.disclinks.partly_mw = lapply(all.disclinks, link.collector)
all.disclinks.partly_mw = unlist(all.disclinks.partly_mw)
all.disclinks[1:10]

# all.stylelinks.collect= lapply(all.stylelinks[1],style.collector)
# all.stylelinks


# all.profiles.partly = unlist(all.disclinks.partly) # transform from list to vector
 profile.links_mw = paste(base.link,all.disclinks.partly_mw, sep ="") # creating full link
 profile.links_mw[1:300] # showing the first 300 links
...
# disc.stats.year.rest3 = profile.links_mw[201:500]  %>%
#   map_df(scrape_discstats)
#  
#  # all.styles.list=all.stylelinks[1:2]  %>%
#  #  map_df(scrape_styles)
# 
#  all.info.500 = profile.links_mw  %>%
#    map_df(scrape_discstats)
#  
# ??stringr
#  disc.500.most.wanted= bind_rows(disc.stats.year.rest3, disc.stats.year.rest2,disc.stats.year.rest)
# # 
?write_csv
??write_csv
 # write_csv( all.info.500,"all_info_500.csv")
 # 
##=========================================================================================
##--------------------------------- 2. Cleaning -------------------------------------------
##=========================================================================================
